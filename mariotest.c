...
int main(void)
{
    ...
    for (j = 0; j < height; j++)
    {
        ...
        for (int l = height; l > j + 1; l--)
        {
            printf(".");
        }
        printf("\n");
    }
    // Print out the number of rows that eventually lead to a shape depending on the nested loop values
    int row;
    for (row = 0; row < height; row++)
    {
        ...
        // To introduce a vertical 2 column gap
        printf("  ");
        
        // Print out the left aligned triangle
        int dothash;
        for (dothash = 0; dothash - 1 < row; dothash++)
        {
            printf("#");
        }
        printf("\n");
